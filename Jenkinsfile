        ...
        stage('Build') {
            steps {
                checkout scm: [
                    ...
                    poll: false  
                script{
                    docker.withRegistry("192.168.41.50:5000","imageRegistry-credential"){   //credential 이름이 jenkins에 등록된 이름과 동일해야 함, jenkins에 docker deploy 권한 필요
                        sh "pwd"
        ...
        stage('GitOps update') {
            steps {
                    ...
                    withCredentials([
                        ...
                        kustomize edit set image 192.168.41.50:5000/${PROJECT_NAME}:${TAG}
                        # 로컬외에는 주석 제거한다
